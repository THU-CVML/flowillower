...
@register_viskit(name="scalar_metrics_dashboard") # Changed decorator
class ScalarMetricsDashboardVisKit(VisKit): # Changed class name and parent
    """
    一个用于显示多个标量指标（每个指标可能包含多个track）的仪表盘视件。
    它会为每个指标在选定的global_step显示st.metric摘要，并绘制一个Plotly折线图。
    A dashboard VisKit to display multiple scalar metrics, each potentially with multiple tracks.
    ...
    # 用于在 data_sources_map 中引用此数据文件的逻辑名称 (由 generate_example_data 返回)
    # Logical name to refer to this data file in data_sources_map (returned by generate_example_data)
    LOGICAL_DATA_SOURCE_NAME = "logged_metrics_source" 
    # 此视件在清单中声明其数据时使用的原始数据类型
    # The original data type this VisKit uses when declaring its data in the manifest
    INTERNAL_DATA_TYPE_NAME = "scalar_dashboard_log_v1"


    def __init__(self,
                 instance_id: str, # Renamed parameter
                 trial_root_path: Path,
                 data_sources_map: Dict[str, Dict[str, Any]],
                 specific_ui_config_dict: Optional[Dict[str, Any]] = None): # Renamed parameter
        ...
        # self.ui_config (类型为ScalarDashboardUIConfig) 已由基类正确初始化
        # self.ui_config (of type ScalarDashboardUIConfig) is correctly initialized by the base class

    @classmethod
    def get_config_model(cls) -> Optional[Type[ScalarDashboardUIConfig]]:
        if PYDANTIC_AVAILABLE:
            return ScalarDashboardUIConfig
        return None 

    @classmethod
    def can_handle_data_types(cls, data_type_names: List[str]) -> bool:
        # 此视件消费由其自身的 report_data 方法生成并标记为 INTERNAL_DATA_TYPE_NAME 的数据
        # This VisKit consumes data generated by its own report_data method and tagged as INTERNAL_DATA_TYPE_NAME
    ...
    def _generate_example_payloads_and_steps(cls,
        ...
        """
        生成用于多次调用 report_data 的示例参数列表。
        Generates a list of example parameter dicts for multiple calls to report_data.
        """
        payloads = []
    ...
    def report_data(self,
        ...
        # 数据文件存储在此视件实例的私有存储中
        # Data file is stored in this VisKit instance's private storage
        log_file_path = self.private_storage_path / self.DATA_FILE_NAME # Use renamed attribute
        ...
        asset_description = {
            ...
            "data_type_original": self.INTERNAL_DATA_TYPE_NAME, # 使用类变量
    ...
    def load_data(self) -> None:
        ...
        if not data_asset_path.exists():
            ...
            self._processed_metrics_data = {}
            self._all_available_steps = [] # 确保在没有数据时步骤列表为空
            return
    ...
    def _render_metric_summary(self, metric_name: str, metric_df: pd.DataFrame, target_step: Optional[int]):
        ...
        # (Logic of this method is same as previous version, relies on self._all_available_steps)
    ...
    def _render_plotly_chart(self, metric_name: str, metric_df: pd.DataFrame, chart_key: str): 
        ...
        # (Logic of this method is same as previous version, relies on self.ui_config.chart_height)
        fig = go.Figure()
        all_tracks = sorted(list(metric_df["track"].unique()))
    ...
    def render_config_ui(self, config_container) -> bool:
        """
        使用 streamlit-pydantic (如果可用) 或手动为 ScalarDashboardUIConfig 渲染配置表单。
        Renders config form for ScalarDashboardUIConfig using streamlit-pydantic (if available) or manually.
        """
        ...
        if ConfigModel and PYDANTIC_AVAILABLE and STREAMLIT_PYDANTIC_AVAILABLE and issubclass(ConfigModel, BaseModel):
            # 使用 streamlit-pydantic
            # Use streamlit-pydantic
            form_key = f"{self.instance_id}_sp_config_form"
            # pydantic_form 需要模型类，它会使用模型中定义的默认值
            # pydantic_form needs the model class, it will use defaults defined in the model.
            # 为了编辑现有实例，我们需要在提交后将返回的数据更新回 self.ui_config
            # To edit an existing instance, we need to update self.ui_config with returned data after submission.
            #
            # 创建一个临时的Pydantic模型实例，其值来自当前的 self.ui_config
            # Create a temporary Pydantic model instance with values from current self.ui_config
            # (这假设 self.ui_config 已经是正确类型的实例，或者可以转换为字典)
            # (This assumes self.ui_config is already an instance of the correct type, or can be cast to dict)
            current_config_dict = {}
            if isinstance(self.ui_config, BaseModel):
                 current_config_dict = self.ui_config.model_dump()
            elif isinstance(self.ui_config, dict): # 如果 self.ui_config 仍然是字典
                 current_config_dict = self.ui_config
            
            # streamlit-pydantic 的 pydantic_form 不直接支持预填充实例进行编辑然后返回bool。
            # 它返回提交后的模型实例，或在未提交时返回None。
            # streamlit-pydantic's pydantic_form doesn't directly support pre-filling an instance for edit and returning bool.
            # It returns the model instance on submission, or None if not submitted.
            # 我们将手动创建表单字段以获得更好的控制。
            # We will create form fields manually for better control.

            with config_container.form(key=f"{self.instance_id}_manual_config_form"):
                new_config_values = {}
                # charts_per_row
                new_config_values["charts_per_row"] = st.slider(
                    "每行图表数", 1, 4, self.ui_config.charts_per_row,
                    key=f"{self.instance_id}_cfg_charts_per_row_manual"
                )
                # chart_height
                new_config_values["chart_height"] = st.number_input(
                    "图表高度", min_value=200, max_value=1000, step=50,
                    value=self.ui_config.chart_height,
                    key=f"{self.instance_id}_cfg_chart_height_manual"
                )
                # show_metric_summary
                new_config_values["show_metric_summary"] = st.checkbox(
                    "显示指标摘要", value=self.ui_config.show_metric_summary,
                ...
                if submitted:
                    try:
                        updated_model = ConfigModel(**new_config_values)
                        if isinstance(self.ui_config, BaseModel) and self.ui_config.model_dump() != updated_model.model_dump():
                            self.ui_config = updated_model
                            changed = True
                        elif not isinstance(self.ui_config, BaseModel): # 如果之前不是模型，现在是
                            self.ui_config = updated_model
                            changed = True
            ...
            config_container.caption("Pydantic或streamlit-pydantic不可用。配置UI需要手动实现。")
    ...
    def render_report_ui(self, report_container) -> Optional[Dict[str, Any]]:
        ...
        with report_container.form(key=f"{self.instance_id}_report_form"):
            ...
            if f"{self.instance_id}_report_metrics" not in st.session_state:
                st.session_state[f"{self.instance_id}_report_metrics"] = [{"metric_name": "loss", "value": 0.0}]

            edited_metrics = st.data_editor(
                st.session_state[f"{self.instance_id}_report_metrics"],
                num_rows="dynamic",
                key=f"{self.instance_id}_report_metrics_editor",
                column_config={
                    "metric_name": st.column_config.TextColumn("指标名称", required=True),
                    "value": st.column_config.NumberColumn("值", format="%.4f", required=True),
                }
            )
            st.session_state[f"{self.instance_id}_report_metrics"] = edited_metrics
            ...
            if submit_button:
                ...
                if not data_payload and valid_payload: 
                    st.warning("请输入至少一个有效的指标和值。")
                    return None
                
                if valid_payload and data_payload:
                    return {
                        "data_payload": data_payload, "step": int(step),
    ...
    def render(self) -> None:
        ...
        with st.expander("视件显示设置 (VisKit Display Settings)", expanded=False): # Changed title
                ...
                with chart_cols[j]:
                    ...
                    with st.container(border=True, height=container_height): 
                        ...
                        if self.ui_config.show_metric_summary:
                            ...
                            st.markdown("---") 
                        chart_key = f"plotly_{self.instance_id}_{metric_name}" # Use self.instance_id
